...
public class HuskySortTest {
    ...
    @Test
    public void Sort1() {
        HuskySort<String> sorter = new HuskySort<>(HuskyCoderFactory.huskySortCoder, false, false);
    ...
    @Test
    public void sort2() {
        String[] words = ChangeToEn.getWords("/shuffledChinese.txt", ChangeToEn::lineAsList, HuskySortTest.class);
        HuskySort<String> sorter = new HuskySort<>(HuskyCoderFactory.huskySortCoder, false, true);
        String[] res = sorter.sort(words);
        String[] sorted = Arrays.copyOfRange(res, 0, 1000000);
        ChangeToEn.writeWords(sorted, String.join(File.separator, "src", "main", "resources", "HuskySort_Result.txt"));
